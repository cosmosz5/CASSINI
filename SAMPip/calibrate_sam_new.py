...
def calibrate_SAM(calibrator_filename, science_filename, delta=2.0):
    ...
    for i in range(len(sc_files)):
        ...
        if len(ind_MJD) == 1:
            ...
            v2_cov, cp_cov = np.cov((cal_v2).T), np.cov((cal_t3phi).T)

            fig1, (ax1, ax2) = plt.subplots(1, 2)
            ax1.imshow(v2_cov, aspect='equal')
            ax2.imshow(cp_cov, aspect='equal')
            plt.show()
            pdb.set_trace()
        ...
        oi_file.vis = oifits.OI_VIS(1, vis_file.dateobs, vis_file.arrname, vis_file.insname, vis_file.target_id,
                                    vis_file.time, vis_file.mjd, vis_file.int_time, calib_v.T, calib_v_err.T,
                                    calib_visphi.T, calib_visphi_err.T, vis_file.ucoord, vis_file.vcoord,
                                    vis_file.sta_index, vis_file.flag)
